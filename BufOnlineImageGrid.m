function GridCovArray=BufOnlineImageGrid(I,GridNum,GridCovArray)
%--------------------------------------------------
%Divide the normalized template into blocks and extract covariance matrix
%as feature to represent each block. This function is used to buffer the
%current estimation for further processing (update the appearance model). 
...
for i = 1:GridNum(1)
    ...
    for j = 1:GridNum(2)
        ...
        GridCovArray{i,j} = [GridCovArray{i,j} Matrix2Vector(logm(eye(6)*1e-5+GPC(I(Idx,Idy),[1 1],...
              [GridSize(1)-2 GridSize(2)-2])),'reduce')];       
    end
end